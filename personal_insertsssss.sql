...
select nombre "Esclavos",cargo_emp "Cargo" from empleados;
select nombre,sal_emp,comision_emp,id_depto FROM empleados WHERE id_depto = 2000 order by comision_emp ASC;
select nombre,(sal_emp+comision_emp+500)"Pago Total",id_depto FROM empleados WHERE id_depto = 3000 order by nombre ASC;
select * FROM empleados WHERE nombre LIKE 'J%' ;
select *,(sal_emp+comision_emp)"Pago Total" From empleados Where comision_emp > 1000;
select *,(sal_emp+comision_emp)"Pago Total" From empleados Where comision_emp = 0;
select * From empleados where comision_emp > sal_emp order by sal_emp ASC;
select * From empleados where comision_emp <= (sal_emp*0.30) order by sal_emp ASC;
...
select * FROM departamentos WHERE nombre_depto NOT in ( "Ventas","Investiganombre_deptoción","Mantenimiento"); -- yo ya sabia paguenme en dolares(funciono de milagro)
select * FROM departamentos WHERE nombre_depto != "Ventas"and nombre_depto !="Investigación" and nombre_depto !="Mantenimiento";
select * FROM departamentos WHERE nombre_depto != "Ventas"and nombre_depto !="Investigación" and nombre_depto !="Mantenimiento" and nombre_jefe_depto != "Bruno Cebrian";
select * FROM departamentos WHERE nombre_depto in ( "Ventas","Investigación","Mantenimiento") and ciudad NOT in ("CIUDAD REAL") ;
-- 21. Mostrar el salario más alto de la empresa.
select * FROM empleados where sal_emp order by sal_emp ASC limit 1;
select max(sal_emp) AS MAYOR FROM empleados;
select max(sal_emp) AS MAYOR FROM empleados;
select * FROM empleados where max(sal_emp);
select * FROM empleados where sal_emp LIKE max(sal_emp);
